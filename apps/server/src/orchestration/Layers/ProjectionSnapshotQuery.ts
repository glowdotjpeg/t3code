...
const ProjectionThreadSearchRequest = Schema.Struct({
  ...
  limit: Schema.Int,
  includeProjectlessThreads: Schema.Boolean,
});
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const searchActiveThreadRows = SqlSchema.findAll({
    ...
    execute: ({ pattern, limit, includeProjectlessThreads }) =>
      sql`
        WITH ranked AS (
          SELECT
          ...
          WHERE threads.deleted_at IS NULL
            ...
            AND (${includeProjectlessThreads ? 1 : 0} = 1 OR threads.project_id IS NOT NULL)
  ...
  const searchThreads: ProjectionSnapshotQueryShape["searchThreads"] = Effect.fn(
    ...
    const rows = yield* searchActiveThreadRows({
      ...
      includeProjectlessThreads: input.includeProjectlessThreads === true,
    }).pipe(
      Effect.mapError(
        toPersistenceSqlOrDecodeError(
