...
export const setCodexSkillEnabled = Effect.fn("setCodexSkillEnabled")(function* (input: {
  ...
  const result = yield* client.request("skills/config/write", {
    path: input.path,
...
export const checkCodexProviderStatus = Effect.fn("checkCodexProviderStatus")(function* (
  ...
  onRateLimits?: (rateLimits: CodexSchema.V2GetAccountRateLimitsResponse) => Effect.Effect<void>,
): Effect.fn.Return<
  ServerProviderDraft,
  ServerSettingsError,
...
> {
  ...
  if (snapshot.rateLimits && onRateLimits) {
    yield* onRateLimits(snapshot.rateLimits);
  }
